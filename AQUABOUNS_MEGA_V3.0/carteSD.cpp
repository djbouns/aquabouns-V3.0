...
const char* pinSurSD = "codepin.txt";
...
NexText*tableauLigneAffichageSd[] = {&valeursd00, &valeursd01, &valeursd02, &valeursd03, &valeursd04
                                    };
...
void lisMesuresEnBinaire24h() {
  ...
  const uint16_t nbr2MesuresPourLeGraphique = (69 + (variable.minut / 20) + 1) ; // 23 heures * 3 mesures par heure + 1 mesure toute les 20 min de l'heure en cours + indexMesure
  ...
  if (myFile) { // fichier ouvert
    ...
    DPRINTF("nbr2MesureMax : "); DPRINT(nbr2MesureMax); DPRINTLN(); // debug
    uint16_t quinzeMinutes = 1200; // 20 minutes en secondes
    uint16_t bufferT = 0;
    uint16_t bufferPh = 0;
    ...
    while (indexMesure < nbr2MesuresPourLeGraphique) { // on boucle autent de fois qu'il faut de pixel pour le graphique
      DPRINTF("indexMesure "); DPRINTLN(indexMesure); // debug
      if (indexMesure >= nbr2MesureMax) { // si index est supperieur au nombre de mesure dispo sur la carte SD
        ...
        DPRINTF("pas assez de mesure pour le graph : "); DPRINTLN(); // debug
      }
      else {
...
void lisMesuresEnBinaire7J() {
  ...
  const byte pixParH = 3; // 4 pixels par heure sur le graph
  const byte TempParPix = 20; // 1 pixel represente 15 min minutes
  const uint16_t PixPour6J = 433; // 6 jours represente 433 pixel sur le graph ( 3 mesures/H * 24h * 6j + indexMesure)
  uint16_t nbr2MesuresPourLeGraphique = (PixPour6J + (variable.Heure * pixParH) + (variable.minut / TempParPix)) ; //
  uint16_t indexMesure = 1; // index des mesures
  myFile = SD.open(mesuresEnBinaire, O_READ); // ouvre le fichier
  if (myFile) { // fichier ouvert
    uint8_t indexErreur = 0; // index pour decaler la lecture si erreur/clef non conforme
...
void stockLesMesures() {
  if (variable.minut == 0 || variable.minut == 20 || variable.minut == 40) { // tout les 1/4 d'heure
...
bool lirePage(pageActuel commande) {
  File fichierLog;
  bool erreur = true;
  static uint8_t ligneSurNextion = 5;
