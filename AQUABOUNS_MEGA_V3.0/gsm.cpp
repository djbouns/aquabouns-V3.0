...
const uint8_t maxPinSize = 4;
char codePin[maxPinSize + 1];
boolean gsmOn = true;
uint8_t signalGSM = 0;
uint32_t compteurEntre2SMS = 0;
const uint16_t delaisEntre2SMS = 30000;
boolean delaisEntre2SMSOK = true;
...
boolean envoyerSMS(const char * noDeTel, const char * messageAEnvoyer) {
  ...
  compteurEntre2SMS = millis();
  if (delaisEntre2SMSOK) {
    if (gsmOn) {
      ...
      if (gsmPrintlnAndWaitATCommand("\"", ">", mille, false)) {
        ...
        delaisEntre2SMSOK = false;
      }
      else {
        gsmPrintlnATCommand(escapeString);
        gsmOn = false;
        DPRINTF("GSM pas pret");  DPRINTLN(); // debug
      }
    }
    else {
    ...
    if (gsmPrintlnAndWaitATCommand(ATString, OKLongString, mille, true)) {// on verifie si le module repond
      ...
      if (gsmPrintlnAndWaitATCommand("AT+CMGF=1", OKLongString, mille, true)) { // on passe les SMS en mode texte
        if (gsmPrintlnAndWaitATCommand("AT+CNMI=1,0,0,0,0", OKLongString, mille, true)) { // on passe en mode routage des SMS vers le terminal
          gsmOn = true;
          DPRINTF("GSM ON ");  DPRINTLN(); // debug
        }
        else { // si erreur passage en mode routage des SMS vers le terminal
          reboot(hardResetGSM); // on reboot
          DPRINTF("reboot Gsm erreur passage mode routage");  DPRINTLN(); // debug
        }
      }
      else { // si erreur passage en mode SMS en mode texte
        reboot(hardResetGSM); // on reboot
        DPRINTF("erreur passage en mode texte");  DPRINTLN(); // debug
      }
    }
    else { // si pas de reponse du module
      reboot(hardResetGSM); // on reboot
      DPRINTF("pas de reponse du GSM");  DPRINTLN(); // debug
    }
  }
  else {
  }
...
void deverouilleSim() {
  ...
  if (strlen(codePin) != maxPinSize ) {
    DPRINTF("probleme de code pin OU pas de code pin"); DPRINTLN(); // debug
  }
  else {
    gsmPrintATCommand("AT+CPIN=");
    gsmPrintlnATCommand(codePin);
  }
  DPRINTF("strlen(codePin) : "); DPRINTLN(strlen(codePin)); // debug
}
