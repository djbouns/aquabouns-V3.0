    ...
    AAAAAAA                 AAAAAAA  QQQQQQQQ::::QQ       UUUUUUUUU   AAAAAAA                 AAAAAAABBBBBBBBBBBBBBBBB        OOOOOOOOO           UUUUUUUUU      NNNNNNNN         NNNNNNN SSSSSSSSSSSSSSS
                                           Q:::::Q
                                            QQQQQQ
                                                             ...
                                                             Et un grand merci a celui qui a redesigner toute la partie graphique de cette Aquabouns V3.0
                                                                                     !!!!!!!!!! Loïc 'Solyum' D. !!!!!!!!!!
...
//                                                      !!!!! Soutenez le projet pour qu'il continu d’évoluer et d’êtres mis a disposition gratuitement !!!!!
//                                                                            https://www.paypal.com/donate/?hosted_button_id=HWQ5X3M39VEP4
...
const char * pageWebVisualisationAquabouns = "/visualistionaquabouns.html";
...
const uint16_t delaisEntreDemandeIdentifiant = 10000; // si pas recu, envoie une demande d'identifiant a l'arduino toute les 10 seconde
...
void envoyerVariables(WiFiClient &client) { // envoie les variable sur la page wew
  ...
  client.print(F("</Trampe><Plight1>"));
  client.print(variable.pwmEclairage[0]);
  client.print(F("</Plight1><Plight2>"));
  client.print(variable.pwmEclairage[1]);
  client.print(F("</Plight2><Plight3>"));
  client.print(variable.pwmEclairage[2]);
  client.print(F("</Plight3><Plight4>"));
  client.print(variable.pwmEclairage[3]);
  client.print(F("</Plight4><Pbrass1>"));
...
void connectionAuWifi() {
  ...
  sprintf (connectionWifi.adresseIP, "%d.%d.%d.%d", WiFi.localIP () [0], WiFi.localIP () [1], WiFi.localIP () [2], WiFi.localIP () [3]); // recupere l'adresse IP
  ...
  while (client.connected()) {
    if (client.available()) {
      ...
      if (c == '\n' && currentLineIsBlank) { // une requête HTTP se termine par une ligne vide
        ...
        else { /*(!strncmp(urlRequest, "/admin", 6))*/   // on vérifie quelle page est demandée en se limitant au 6 premier caracter
          if (SPIFFS.exists(pageWebVisualisationAquabouns)) {
            // On envoie un en tête de réponse HTTP standard de type HTML
            client.println("HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: keep-alive\r\n");
            File pageWeb = SPIFFS.open(pageWebVisualisationAquabouns, "r");
        ...
        /* else {
           if (SPIFFS.exists(pageWebFavicon)) {
             // On envoie un en tête de réponse HTTP standard de type HTML
             File pageWeb = SPIFFS.open(pageWebFavicon, "r");
             client.write(pageWeb);
             pageWeb.close();
           } else {
